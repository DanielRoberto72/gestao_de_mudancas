...
tempo_1dias = datetime.now() - timedelta(1)
timestamp = tempo.strftime('%d-%m-%Y')
timestamp_envio = tempo.strftime('%d-%m')
timestamp_1dias = tempo_1dias.strftime('%d/%m/%Y')
...
if(dfNoc.empty is False):
    try:
        ...
        password = ''
        send_to_email = []
        subject = ': '+timestamp_1dias

        html = """\
        <html>
        <head>
        <style> 
        table, th, td {{font-size:12pt; border:4px solid white; border-collapse:collapse; text-midle:;}}
        ...
        <p>Segue o resultado das GMUD's:</p><br>
            {0}
        ...
        """.format(dfNoc.to_html(index=False, border=4, justify=CENTER, col_space=137, classes='mystyle' ))
    ...
    print('Não houve Gmuds finalizadas na noite anterior')